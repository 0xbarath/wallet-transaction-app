...
A Spring Boot service that registers EVM wallet addresses, syncs on-chain transaction history, and lets users query it through structured filters or natural-language prompts. It fetches transfers via Alchemy's Asset Transfers API, enforces role-based access control (admin vs user), and provides an admin-only transaction enrichment endpoint that classifies DeFi operations (Aave, Uniswap, Lido, etc.) with optional LLM-powered explanations grounded strictly in on-chain evidence.
...
- **Anthropic API key** — required for LLM-powered transaction explanations (`explain=true`). Set `ANTHROPIC_API_KEY` in your environment. The explain feature is disabled automatically when the key is absent.
  ...
  "items": [
    {
      ...
      "blockNum": 19456789,
      "blockTs": "2025-03-15T14:23:01Z",
      "fromAddr": "0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045",
      "toAddr": "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48",
      "direction": "OUT",
      "asset": "USDC",
      "category": "ERC20",
      "valueDecimal": 2500.00,
      "rawValue": "2500000000",
      "rawContractAddr": "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48",
      "rawContractDecimals": 6,
      "tokenId": null,
      "createdAt": "2025-03-15T14:23:01Z"
    },
    {
      ...
      "category": "INTERNAL",
      "valueDecimal": 0.5,
      "rawValue": "500000000000000000",
      "rawContractAddr": null,
      "rawContractDecimals": null,
      "tokenId": null,
      "createdAt": "2025-03-15T13:45:30Z"
    }
  ],
...
Note: admin responses include `INTERNAL` category transfers and all raw contract fields.
...
The security filter chain enforces this in order:
1. **AuthFilter** — validates `X-Auth-WalletAccess: allow`
2. **RbacFilter** — extracts and validates `X-Role`
3. **AdminEnrichmentFilter** — blocks non-admin access to `/v1/transactions/explain`
...
**Access control**: Guarded by RBAC at two layers — the `AdminEnrichmentFilter` rejects non-admin requests with `403 Forbidden` before reaching the controller, and the service layer performs a defence-in-depth check. Calling without `X-Role: admin` returns:
